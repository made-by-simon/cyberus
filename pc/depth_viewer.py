#!/usr/bin/env python3
"""
Real-time stereo + depth viewer — runs on your PC.
Connects to the Jetson depth server and displays left | right | depth.
...
class DepthViewer:
    ...
    def run(self):
        ...
        while True:
            ...
            # labels
            cv2.putText(frame, 'Left',          (8, 28),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(frame, 'Right',         (third + 8, 28),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(frame, 'Depth (MiDaS)', (2 * third + 8, 28),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
            # FPS + latency
